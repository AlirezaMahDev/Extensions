...
public partial class ProgressStatus(ILogger? logger, Action<ProgressStatusState>? action = null, int length = -1)
    : Progress<ProgressStatusState>(action ?? (_ => { }))
{
    private int _count;

    public ProgressStatusState State { get; private set; } = ProgressStatusState.Empty;
    ...
    protected override void OnReport(ProgressStatusState value)
    {
        if (logger != null)
        {
