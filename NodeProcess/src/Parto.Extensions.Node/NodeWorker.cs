    ...
    where TNodeServiceOptions : NodeOptions
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var workingDirectory = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)!,
            _options.Assembly.GetName().Name!);
        _process.StartInfo = new("node", $"{_options.Path} {config}")
        {
